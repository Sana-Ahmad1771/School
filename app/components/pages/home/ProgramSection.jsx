        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-16 gap-6">
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[#9C1D20] font-bold uppercase tracking-widest text-xs">Academic Structure</span>
            </div>
          ...
          <Link href="/academics">
            <button className="flex items-center gap-2 bg-[#1F1A55] text-white px-8 py-5 rounded-full font-bold hover:bg-[#15123d] transition-all text-sm uppercase tracking-wider shadow-lg shadow-[#1F1A55]/20 group">
              ...
              <div key={index} className="border-b border-gray-200">
                ...
                >
                  <div className="flex items-center gap-6 md:gap-12">
                    <span className="text-lg font-bold text-[#9C1D20]/40 font-mono">0{index + 1}</span>
                    <h3 className={`text-3xl md:text-[42px] font-bold tracking-tight transition-all duration-300 ${isOpen ? 'text-[#9C1D20]' : 'text-[#1F1A55] group-hover:translate-x-2'}`}>
                      {item.name}
                    </h3>
                  </div>
                  
                  <div className={`relative flex items-center justify-center w-12 h-12 md:w-16 md:h-16 rounded-full border transition-all duration-300 ${isOpen ? 'bg-[#9C1D20] border-[#9C1D20] text-white rotate-180' : 'border-gray-300 text-[#1F1A55] group-hover:bg-[#1F1A55] group-hover:text-white'}`}>
                ...
                <div 
                  className={`overflow-hidden transition-all duration-500 ease-in-out ${
                    isOpen ? "max-h-[800px] opacity-100 mb-14" : "max-h-0 opacity-0"
                  }`}
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-12 px-4 md:pl-24">
                    <div className="flex flex-col justify-center">
                      ...
                      <div className="flex flex-wrap gap-3 mb-10">
                        {item.tags.map(tag => (
                          <span key={tag} className="px-4 py-1.5 bg-gray-100 text-[#1F1A55] rounded-full text-xs font-bold uppercase tracking-wide">
                            {tag}
                          </span>
                        ))}
                      </div>

                      <Link href="/admissions" className="flex items-center gap-2 font-bold text-[#1F1A55] border-b-2 border-[#1F1A55] w-fit pb-1 hover:gap-4 transition-all uppercase text-xs tracking-widest">
                        Apply for this level <ArrowUpRight size={16} />
                      </Link>
                    </div>
                    
                    <div className="h-[350px] w-full overflow-hidden rounded-2xl shadow-2xl">
                      <img 
                        src={item.image} 
                        alt={item.name} 
